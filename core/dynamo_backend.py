...
@author: maxbi
"""
from smarterbmsdb.utilities import timestamp2unix, getDtypes
import boto3
...
def main():
    unix_start = timestamp2unix("12:00 10/10/2021")
    unix_end = timestamp2unix("12:00 12/11/2021")
    freq = 60*15
    
    table = dynamoTable("bmsTrial", "C:/Users/maxbi/Documents/git_repos/smarter_bms_controller/cloudcontroller/data_collected_in_DynamoDB.xlsx")
    ...
    # query a specific topic for the time period [strStart, strEnd]
    # topic_data = table.getTopicData('kings-lynn/OS-23/sensor/ave-sales-temp-C', unix_start, unix_end, freq)
    
    # query a groups of topics for the time period [strStart, strEnd]. Can either be list of topic strings, or string of the outstation name. Can also specify a resampledInterval parameter to resample the data. 
    # topics_data = table.getTopicsData(topic_list, unix_start, unix_end, freq)
    
    # query a group of energy meters for the time period [strStart, strEnd]. Can either be a list of meter topic strings, or default behaviour returns data for all meters.
    # meterReadings = table.getEnergyMeters(topic_list, unix_start, unix_end, resample_interval="15min")
    
    # Query weather data
    # attributes = ["apparentTemperature"] # "all" or list of weather types
    # forecast_horizon = 12
    # pk = "weather"
    # df = table.queryWeatherOrCarbon(pk, unix_start, unix_end, attributes, forecast_horizon)
    
    # Query carbon data
    attributes=["carbonFactor"] # "all" or list of weather types
    forecast_horizon = 23
    pk = "carbon"
    df = table.queryWeatherOrCarbon(pk, unix_start, unix_end, attributes, forecast_horizon)
    
    # query last known value for a list of topics.
    # response = table.lastKnownValue(topic_list)

    return df

if __name__ == "__main__":
    topics_data = main()
    # table = dynamoTable("bmsTrial", "C:/Users/maxbi/Documents/git_repos/smarter_bms_controller/backend/data_collected_in_DynamoDB.xlsx")
